...
module.exports = {
  ...
  async BuscarUbicacionStockInicial(UbicacionStock) {
    //I.wait(5);
    ProductoStockInicial = await I.retry(wait).grabTextFrom(
      locator.stockInicial(UbicacionStock)
    );
  },
  ...
  async AgregarCantidadProductoLanding(cantidadItem) {
    ...
    await I.retry(wait).fillField(
      locator.inputCantidadItemLanding,
      cantidadItem
    );
  ...
  async ObtenerCajaStockFinal(UbicacionStock) {
    ...
    var StockActual = await I.retry(wait).grabTextFrom(
      locator.stockInicial(UbicacionStock)
    );
